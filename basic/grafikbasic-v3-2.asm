;  * * AC1 * * GRAFIK/SOUND-BASIC V.3.2 * (C)1978 Microsoft & * *
;  * * (C)1988 E.Ludwig * * 

; File Name   :	P.Grafik Basic 3.2_4000_5FFF_0000.bin
; reassembled: V.Pohlers, 11/2018, 02/2023
; last change: 
...
; gepatchte Version f¸r Modul1

		cpu	z80undoc


basver	equ	0		; 0 - orig, 1 - gepatchte Version von Modul1
...
byte_6009	equ	6009h	; genutzt f. MODE 1+2
...
MO      EQU    19H             ; Missing operand

		org 4000h


COLD:		ld	hl, loc_5CFD	; NMI-Routine
		ld	(unk_1818), hl	; nmi-sprungadr
		...
		ex	de, hl
		ld	hl, INITAB
		ld	c, 5Fh ; b ist noch 0
		ldir
		ex	de, hl
		ld	sp, hl
		call	CLREG
		ld	(unk_60A6), a	; ?
		ld	(PROGST), a	; Start	of program text	area
...
SETTOP:		LD      DE,0-50         ; 50 Bytes string space
...
BRKRET: 	CALL    CLREG           ; Clear registers and stack
		...
		if basver = 0
MEMMSG:		db 0Ch,0Dh," * * AC1 * * GRAFIK/SOUND-BASIC V.3.2 * (C)1978 Micr"
		db "osoft & * *",0Dh,0Fh," * * (C)1988 E.Ludwig * * ",0
		else
MEMMSG:		db 0Ch,0Dh," ** AC1 * GRAFIK/SOUND-ROM-BASIC V.3.2 * (C)1978 Mic"
		db "rosoft & **",0Dh,0Fh," * * (C)1988 E.Ludwig * * ",0
		endif

BFREE:		db " Bytes free *",0Dh,0
...
;--
	DB   1               ; POS (x) number (1)
	DB   0FFh            ; Terminal width (255 = no auto CRLF)
	...
	db    0
	db    0
	JP    unk_4577          ; Input reflection (set to TTY)
	db    0
	db    0
	db    0
	db    0
...
ERRORS: DB   "NF"            ; NEXT without FOR
	...
	DB   "LD"            ; Load Fehler infolge RAM-Fehler
...
ERRIN:  CALL    PRS             ; Output message
	...
	JP      Z,COLD          ; Cold start error - Restart
...
unk_4577:	ld	c,1
		ld	hl, BUFFER
		ld	b, 1
loc_457E:	call	unk_8	; inch
		cp	14h
		jr	c, loc_4589
		cp	1Ch
		jr	c, loc_45DF
loc_4589:	call	sub_5CCF
loc_458C:	jr	nz, loc_457E
		ret	c
		push	de
		ld	de, (cupos)
		ld	a, e
...
loc_45D5:	ld	a, e
		...
		jp	ENDINP
loc_45DF:	push	hl
		ld	hl, unk_1F80	; Funktionstasten
		sub	13h
		ld	b, a
loc_45E6:	ld	a, (hl)
		inc	hl
...
CLREG:  POP     BC              ; Save return address
	LD      SP,(STRSPC)     ; Set stack end of working RAN
...
LISTLP: POP     HL              ; Restore address of line
	...
	OR      C
	Jr      Z,loc_473C        ; Yes - Go to command mode
...
;CALL* 1F00 dto. ab Adresse $1F00 Hexadezimal
PCALL:	call	sub_5ABE
	jr	z, loc_5AB5
	call	GETNUM
	call	DEINT
...
;RENUMBER10,50, Nummeriert die Zeilen 10 bis 50 neu, 
; neue 200,5 Zeile 200 Abstand 5
RENUM:	push	hl
	ld	hl, 10
	ld	(unk_6054), hl
	ld	hl, (BASTXT)
...
;------------------------------------------------------------------------------
; Erweiterungen Basic V3.2 gegen¸ber Original
;------------------------------------------------------------------------------
...
INKEY:		call	IDTEST
		...
		inc	hl
		ld	(hl), d
		call	sub_5FC4	; jp unk_297 UP "Taste"
		pop	hl
		ret	z
		ld	(de), a
		dec	de
...
KEY:		jr	nz, LOCATE	; wenn noch wasl folgz, dann Sprung
					; aber warum zu LOCATE und nicht zu ERROR???
		;			
		ld	bc, 831h	; B=8, C='1'
		ld	de, unk_1F80+1	; Funktionstasten
loc_5D11:	rst	18h
		db 0Dh,"KE",'Y'+80h
		ld	a, c
		rst	10h		; outch nummer
		rst	18h		; Leerzeichen
		db 0A0h
loc_5D1A:	ld	a, (de)
		inc	de
		cp	' '		; Steuerzeichen?
		jr	nc, loc_5D22	; nein
		add	a, 0E0h		; Sonst stattdessen Grafikzeichen ausgeben
loc_5D22:	cp	0E0h		; 00-Byte
		call	nz, OUTC	; dabei ¸bergehen
		jr	nz, loc_5D1A	; 00 ist Trennzeichen zu n‰chstem Key
		inc	c		; n‰chste Nummer
		djnz	loc_5D11	; 8 Funktionstasten durchlaufen
		ret
...
LOCATE:		call	CHKSYN
		...
		cp	32
		pop	de		; d=Spalte
loc_5D43:	jp	nc, FCERR
		ld	b, 6		; 2^6 = 64
		ld	e, d		; e=Spalte
		ld	d, 0		; da=Zeile
loc_5D4B:	sla	a
		rl	d		
		djnz	loc_5D4B	; da=Zeile * 64
		add	a, e		; + Spalte 
		ld	e, a		; de=pos
		push	hl
		ld	hl, 17FFh	; BWS links oben
		sbc	hl, de		; Pos. Berechnen
		ld	(cupos), hl	; Cusor setzen
loc_5D5C:	pop	hl
		call	CHKSYN
		db ')'
		ret
...
;-------------------
;Raster - Grafik :
;Der Bildschirm wird in 8192 Bildpunkte unterteilt (128 x 64)
;Jeder Bildpunkt kann gesetzt, r¸ckgesetzt und getestet werden.
;SET (x,y) SET (63,31) setzt Punkt in Bildmitte
;RESET (x,y) RESET (A,B)
;POINT (x,y) POINT (0,0) testet links oben
;x=0 Bis 127; y=0 bis 63
...
;
;-------------------
; CSAVE"Name" Speichert das Basicprogramm auf Kassette, Name 1-16 Zeichen
; CSAVE*"Name" Speichert Feld unter Name auf Kassette
CSAVE:		cp	ZTIMES		; folgt '*'?
		jr	z, loc_5DEF
;Programm speichern
		call	EVAL
		push	hl
		ld	c, 'B'          ; Basic-Programm
		ld	hl, (PROGND)
		ld	de, (BASTXT)
		call	sub_5F39	; tb-Kopfblock f¸llen
		call	sub_5F00	; speichern
loc_5DEB:	pop	hl
		jp	unk_A31		; q_ko20, Start/Stop-Schaltung stop
; Feld speichern		
loc_5DEF:	inc	hl
		call	EVAL
		push	hl
		...
		call	sub_5F39	; tb-Kopfblock f¸llen
		call	sub_5F00	; speichern
		call	sub_5F2C	; Speicherbereich Variablen selektieren
		call	unk_BF7		; tbsave
		jr	loc_5DEB
;
;-------------------
; CLOAD"Name" L‰dt ein Programm von Kassette
; CLOAD* A$ L‰dt Feld mit ¸bereinstimmenden Namen in A$ von Kassette.
CLOAD:		cp	ZTIMES		; folgt '*'?
		jr	z, loc_5E4D
; Programm laden
		ex	af, af'		; '
		call	EVAL
		...
		call	sub_5E70
		jr	nc, loc_5E48
		; Programm hinter aktuelles Programm laden
		ld	hl, (unk_18D1)	; tbl_eadr
		ld	de, (unk_18D3)	; tbl_aadr
		and	a
		sbc	hl, de		; hl=l‰nge
		ld	de, (PROGND)	; aktuelles Programmende
		dec	de		; 2 Zeichen zur¸ck (Zeilennummer 0000)
		dec	de		; 
		add	hl, de		; neues Programmende
		call	ENFMEM		; test auf genug speicher
		ld	(unk_18D3), de	; tbl_aadr  neue Ladeadressen setzen
		ld	(unk_18D1), hl	; tbl_eadr
		ld	(PROGND), hl
loc_5E48:	call	loc_5ED3
loc_5E4B:	jr	loc_5DEB
;Feld laden
loc_5E4D:	inc	hl
		and	a
		ex	af, af'		;'
		...
		jr	loc_5E4B
;
sub_5E70:	call	unk_A37		; q_ko21, Start/Stop-Schaltung start
		ld	(unk_1880), hl	; tbb_aadr
		ld	l, 0D5h		; ???
		ld	(unk_187E), hl	; tbb_eadr
		rst	18h
		db "Search for",0A0h
		ld	l, lo(unk_189E)	; tbh_filename, h=18h
		push	bc
		ld	b, 10h
		call	sub_5ECB
loc_5E8F:	call	unk_C81		; tbload
		ld	hl, unk_18B6	; tb_blkbuf
		ld	de, unk_1896	; tb_kopfbuf
		ld	b, 8
loc_5E9A:	ld	a, (de)
		cp	(hl)
		jr	nz, loc_5E8F
...
loc_5EB8:	rst	18h
		db "Found  ",0A0h
		call	sub_5EC6	; Anzeige Filename
		jr	loc_5E8F

; Anzeige Filename
sub_5EC6:	ld	hl, unk_18BE	; tbl_filename
		ld	b, 13h		; 16+2+1 (Filename, Trenner, Dateityp)
sub_5ECB:	ld	a, (hl)
		rst	10h
		inc	hl
		djnz	sub_5ECB
...
; Adressen aus Lese-Kopfblockbuffer ¸bernehmen und Laden
loc_5ED3:	ld	hl, unk_18D1	; tbl_eadr
		ld	de, unk_187E	; tbb_eadr
		ld	bc, 4
		ldir
		rst	18h
		db "Loading",0A0h
		call	sub_5EC6	; Anzeige Filename
sub_5EEA:	call	unk_C81		; tbload
		ld	a, iyl		; IY zeigt in Fehlerarray
		sub	82h		; lo(tb_errarr) auf anfang?
		ret	z		; dann kein Lesefehler, also ok
		rrca			; 
		exx
		cp	l
		call	nz, unk_183	; OUTHEX
		call	unk_A31		; q_ko20, Start/Stop-Schaltung stop
		ld	e, 24h 		; "LD" load error
		jp	ERROR
		
; speichern
;in hl = 18h
sub_5F00:	call	unk_A37		; q_ko21, Start/Stop-Schaltung start
		dec	hl		; h==18h
		ld	(unk_187E), hl	; tbb_eadr
		ld	l, lo(unk_1896)		; lo(tb_kopfbuf)
		ld	(unk_1880), hl	; tbb_aadr
		rst	18h
		db "Saving",0A0h
		ld	l, lo(unk_189E)  	; lo(tbh_filename)
		ld	b, 10h
		call	sub_5ECB
		call	unk_BF7		; tbsave
		ld	hl, unk_18B1	; tbh_eadr
		ld	de, unk_187E	; tbb_eadr
		ld	bc, 4
		ldir
		jp	unk_BF7		; tbsave

; Speicherbereich Variablen selektieren
sub_5F2C:	ld	hl, (LSTRAM)	; Last available RAM
		ld	(unk_187E), hl	; tbb_eadr
		ld	hl, (STRBOT)	; Bottom of string space
		ld	(unk_1880), hl	; tbb_aadr
		ret
...
; tb-Kopfblock f¸llen
sub_5F39:	push	bc
		ld	(unk_18B1), hl	; tbh_eadr
		ld	(unk_18B3), de	; tbh_aadr
		call	GETLEN
		jp	z, FCERR
		inc	hl
		inc	hl
...
loc_5F60:	inc	de
		...
		djnz	loc_5F59
		pop	bc
		jp	unk_BD4		; weiter im Monitor bei tbh6

;-------------------
;SOUND X,Y Tonausgabe X= Tonl‰nge, Y= Tonhˆhe
SOUND:		jp	z, unk_272	; beep, UP "akustisches Signal"
		call	GETINT
		push	af
		call	CHKSYN
		inc	l
...
loc_5F8C:	srl	b
		...
		sub	b
loc_5F93:	ld	b, d
		call	unk_287	; UPTON, UP "Ton", Reg, B = Tonlaenge, C = Tonhoehe
		ld	b, d
		call	unk_287	; UPTON, UP "Ton", Reg, B = Tonlaenge, C = Tonhoehe
		dec	a
		jr	nz, loc_5F93
...
;-------------------
;MODE Wechselt Zeichensatz ZG (GRAFIK-AC1)
; MODE 0 Zeichensatz 1 Grafikmodus 
; 	Befehle wirken auf Hardwareerweiterung
; 	4/8K ZG, ohne Erweiterung ist st‰ndig
; 	Grafikmodus, MODE und MODE 0 ohne Wirkung.
; MODE 1 FAST-Modus des Basicinterpreter, Arbeitsgeschwindigkeit
; 	des nachfolgenden Programm ca. doppelt
; MODE 2 SLOW-Modus, normale Arbeitsgeschwindigkeit
MODE:		jr	nz, loc_5FA8	; folgt Parameter?
		; nein, dann Wechsel Zeichensatz
		in	a, (5)
		xor	8
		out	(5), a
		ret
;
loc_5FA8:	call	GETINT		; Parameter
		and	a
		jr	z, loc_5FD9	; mode 0
		; mode 1
		ld	(byte_6009), a	; fastmode=1/2
		ret

;-------------------
;CLS Lˆscht Bildschirm
CLS:		ld	a, 0Ch
;
MONOUT:		push	hl
		ld	hl, kdov24
...
; UP zu TSTBRK
sub_5FBF:	ld	a, (byte_6009)	; fastmode
		dec	a		; =1?
		ret	z		; dann raus
					; sonst Test auf Taste
sub_5FC4:	jp	unk_297	; UPTAST, UP "Taste", testet Tastaturtatus
...
;Start des Basic-Interpreters V. 3.2
		db 0,9,'b',0Dh
		ld	hl, COLD
		jr	loc_5FD8
;
; Start des Basicinterpreter V.3.2 (warm)
		db 0,9,'r',0Dh
		
	else
; patches f¸r Modul-1 Paket X V.2
loc_5FC7:	LD      HL, COLD
		JR      loc_5FD8
		;
		JR      loc_5FC7
		;
loc_5FCE:	LD      BC,RUNCNT
		PUSH    BC
		JP      RUNFST
