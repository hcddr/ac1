...
; verändert auf Version "AC1-2010 * FARBMONITOR * FDC/RFL"
; verändert auf Version "AC1-2010 * MONITOR * 12/2011"
...
init6:		; Sprungverteiler initialisieren
		ld	bc, lo(sv_rst)	; == Länge sv_rst-Bereich
...
; verhindert werden, indem beim Einschalten bzw. RESET die Taste "CR" gedrückt
; wird: Sprung in den Monitor, oder Taste "X" gedrückt: Sprung in Programmpaket X.
;------------------------------------------------------------------------------

rWarm:		call	UPTAST		; Taste bei Einschalten gedrückt?
		jr	nz, rWarm1
...
crc:		equ	$-1		; vp: Adresse-1 ist ein Fehler
					; wird 2x angesprungen! passiert glücklicherweise nix...
...
loc_743:	call	REGA		; CPU-Register ausräumen nach RSA
		ld	hl, (loc_235)	; wieder BREAK eintragen, Std. Routine RST 38
					; vp: müsste daher eigentlich loc_235+1 sein !!!
		ld	(jp_rst38+1), hl	; Eintragen bei RST 38
		pop	hl
		dec	hl
...
;------------------------------------------------------------------------------
; > ändert beliebige Anzahl von Byte ab Adresse
;------------------------------------------------------------------------------
...
; 'U', Kennbyte für Namen
; Namen, max. 16 Zeichen, wenn kürzer wird mit Leerzeichen aufgefüllt
; 216 Nullbytes, Zeit für Namensdarstellung beim Einlesen
; es folgen Blöcke:
; 	'<', Kennbyte für Datenblock
; 	XX, Blocklänge, normal 256, Restblock entsprechend weniger
; 	XXXX, Blockstartadresse
; 	256 Datenbyte, Restblock entsprechend weniger
; 	0XX, Prüfsumme
; 'x' Kennbyte für Endeblock
;  XXXX Startadresse
;------------------------------------------------------------------------------
